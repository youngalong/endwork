...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, style_input=False, noise_mode='random', fused_modconv=True, gain=1, noise_buf=None):
        assert noise_mode in ['random', 'const', 'none', 'injected']
        ...
        if self.use_noise and noise_mode == 'injected':
            noise = noise_buf * self.noise_strength
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, style_input=False, force_fp32=False, fused_modconv=None, noise_bufs=None,
                **layer_kwargs):
        if noise_bufs is not None:
            noise_iter = iter(noise_bufs)
        else:
            noise_iter = iter([None, None])
        if not style_input:
        ...
        # Main layers.
        if self.in_channels == 0:
            x = self.conv1(x, next(w_iter), style_input=style_input, fused_modconv=fused_modconv,
                           noise_buf=next(noise_iter), **layer_kwargs)
        elif self.architecture == 'resnet':
            y = self.skip(x, gain=np.sqrt(0.5))
            x = self.conv0(x, next(w_iter), style_input=style_input, fused_modconv=fused_modconv,
                           noise_buf=next(noise_iter), **layer_kwargs)
            x = self.conv1(x, next(w_iter), style_input=style_input, fused_modconv=fused_modconv,
                           noise_buf=next(noise_iter), gain=np.sqrt(0.5), **layer_kwargs)
            x = y.add_(x)
        else:
            x = self.conv0(x, next(w_iter), style_input=style_input, fused_modconv=fused_modconv,
                           noise_buf=next(noise_iter), **layer_kwargs)
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, style_input=False, return_features=False, noise_bufs=None, **block_kwargs):
        block_ws = []
        ...
        if not style_input:
            for i, (res, cur_ws) in enumerate(zip(self.block_resolutions, block_ws)):
                block = getattr(self, f'b{res}')
                if noise_bufs is not None:
                    block_kwargs = {**block_kwargs, 'noise_bufs': noise_bufs[block_idx: block_idx + block.num_conv]}
                    block_idx += block.num_conv
