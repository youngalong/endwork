...
class SpaceRegularizer:
    ...
    def ball_holder_loss_lazy(self, new_G, num_of_sampled_latents, w_batch, log_name, use_wandb=False):
        ...
        w_samples = self.original_G.mapping(torch.from_numpy(z_samples).to('cuda:0'), None,
                                            truncation_psi=0.5)
        ...
        for w_code in territory_indicator_ws:
            ...
            if regularizer_l2_lambda > 0:
                l2_loss_val = l2_loss.l2_loss(old_img, new_img)
                if use_wandb:
                    wandb.log({f'losses/space_regularizer_l2_loss_val_{log_name}': l2_loss_val.detach().cpu()},
                              commit=False)
                loss += l2_loss_val * regularizer_l2_lambda

            if regularizer_lpips_lambda > 0:
                loss_lpips = self.lpips_loss(old_img, new_img)
                loss_lpips = torch.mean(torch.squeeze(loss_lpips))
                if use_wandb:
                    wandb.log({f'losses/space_regularizer_lpips_loss_val_{log_name}': loss_lpips.detach().cpu()},
                              commit=False)
                loss += loss_lpips * regularizer_lpips_lambda
